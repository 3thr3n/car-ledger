// This file is auto-generated by @hey-api/openapi-ts

export type ClientOptions = {
  baseUrl: 'http://localhost:8080' | 'http://0.0.0.0:8080' | (string & {});
};

export type Account = {
  maxCars?: number;
  name?: string;
};

export type AverageStats = {
  pricePerUnit?: number;
  distance?: number;
  calculated?: number;
  calculatedPrice?: number;
};

export type Bill = {
  id?: number;
  day?: LocalDate;
  year?: number;
  distance?: number;
  unit?: number;
  pricePerUnit?: number;
  estimate?: number;
  calculated?: number;
  calculatedPrice?: number;
};

export type BillInput = {
  day?: LocalDate;
  distance?: number;
  unit?: number;
  pricePerUnit?: number;
  estimate?: number;
};

export type BillPaged = {
  total?: number;
  page?: number;
  size?: number;
  data?: Array<Bill>;
};

export type Car = {
  id?: number;
  name?: string;
  year?: number;
  odometer?: number;
  amountBills?: number;
};

export type CarInput = {
  name?: string;
  year?: number;
  odometer?: number;
};

export type CarOverview = {
  totalRefuels?: number;
  totalCost?: number;
  avgConsumption?: number;
};

export type CsvOrder = {
  day?: number;
  unit?: number;
  pricePerUnit?: number;
  distance?: number;
  estimate?: number;
};

export type HiLo = {
  min?: number;
  max?: number;
};

export type HiLoStats = {
  calculatedPrice?: HiLo;
  calculated?: HiLo;
  distance?: HiLo;
  unit?: HiLo;
  pricePerUnit?: HiLo;
};

export type LocalDate = string;

export type MinimalStats = {
  total?: number;
  avgFuelConsumption?: number;
  minMaxFuelConsumption?: HiLo;
  avgDistance?: number;
};

export type TotalStats = {
  unit?: number;
  distance?: number;
  calculatedPrice?: number;
};

export type HiLoWritable = {
  min?: number;
  max?: number;
  scale?: number;
};

export type HiLoStatsWritable = {
  calculatedPrice?: HiLoWritable;
  calculated?: HiLoWritable;
  distance?: HiLoWritable;
  unit?: HiLoWritable;
  pricePerUnit?: HiLoWritable;
};

export type MinimalStatsWritable = {
  total?: number;
  avgFuelConsumption?: number;
  minMaxFuelConsumption?: HiLoWritable;
  avgDistance?: number;
};

export type CallbackData = {
  body?: never;
  path?: never;
  query?: never;
  url: '/api/auth/callback';
};

export type CallbackResponses = {
  /**
   * OK
   */
  200: unknown;
};

export type LoginData = {
  body?: never;
  path?: never;
  query?: never;
  url: '/api/auth/login';
};

export type LoginResponses = {
  /**
   * OK
   */
  200: unknown;
};

export type LogoutData = {
  body?: never;
  path?: never;
  query?: never;
  url: '/api/auth/logout';
};

export type LogoutResponses = {
  /**
   * OK
   */
  200: unknown;
};

export type LogoutCallbackData = {
  body?: never;
  path?: never;
  query?: never;
  url: '/api/auth/logout-callback';
};

export type LogoutCallbackResponses = {
  /**
   * OK
   */
  200: unknown;
};

export type AddNewBillData = {
  body: BillInput;
  path: {
    carId: number;
  };
  query?: never;
  url: '/api/bill/{carId}';
};

export type AddNewBillErrors = {
  /**
   * Car is not for your user.
   */
  400: unknown;
  /**
   * Something went wrong while saving. Please ask the server admin for help.
   */
  500: unknown;
};

export type AddNewBillResponses = {
  /**
   * Bill created.
   */
  200: unknown;
};

export type GetAllBillsData = {
  body?: never;
  path: {
    carId: number;
  };
  query?: {
    page?: number;
    size?: number;
    year?: number;
  };
  url: '/api/bill/{carId}/all';
};

export type GetAllBillsResponses = {
  /**
   * Bills found.
   */
  200: BillPaged;
};

export type GetAllBillsResponse =
  GetAllBillsResponses[keyof GetAllBillsResponses];

export type GetAllBillYearsData = {
  body?: never;
  path: {
    carId: number;
  };
  query?: never;
  url: '/api/bill/{carId}/years';
};

export type GetAllBillYearsResponses = {
  /**
   * Bills found and years extracted.
   */
  200: Array<number>;
};

export type GetAllBillYearsResponse =
  GetAllBillYearsResponses[keyof GetAllBillYearsResponses];

export type DeleteBillData = {
  body?: never;
  path: {
    billId: number;
    carId: number;
  };
  query?: never;
  url: '/api/bill/{carId}/{billId}';
};

export type DeleteBillErrors = {
  /**
   * Bill not found on the specified car.
   */
  400: unknown;
  /**
   * Something went wrong while deleting. Please ask the server admin for help.
   */
  500: unknown;
};

export type DeleteBillResponses = {
  /**
   * Bill deleted.
   */
  202: unknown;
};

export type GetMyCarsData = {
  body?: never;
  path?: never;
  query?: never;
  url: '/api/car/my';
};

export type GetMyCarsResponses = {
  /**
   * Get all cars.
   */
  200: Array<Car>;
};

export type GetMyCarsResponse = GetMyCarsResponses[keyof GetMyCarsResponses];

export type CreateCarData = {
  body: CarInput;
  path?: never;
  query?: never;
  url: '/api/car/my';
};

export type CreateCarErrors = {
  /**
   * Maximal amount of cars created or input was invalid.
   */
  400: unknown;
  /**
   * Something went wrong while saving. Please ask the server admin for help.
   */
  500: unknown;
};

export type CreateCarResponses = {
  /**
   * Car created.
   */
  202: unknown;
};

export type GetMyCarData = {
  body?: never;
  path: {
    id: number;
  };
  query?: never;
  url: '/api/car/my/{id}';
};

export type GetMyCarResponses = {
  /**
   * Car found.
   */
  200: Car;
  /**
   * Id was not found.
   */
  204: void;
};

export type GetMyCarResponse = GetMyCarResponses[keyof GetMyCarResponses];

export type UpdateMyCarData = {
  body: CarInput;
  path: {
    id: number;
  };
  query?: never;
  url: '/api/car/my/{id}';
};

export type UpdateMyCarErrors = {
  /**
   * Input was invalid.
   */
  400: unknown;
};

export type UpdateMyCarResponses = {
  /**
   * Car found and updated.
   */
  200: unknown;
};

export type GetMyCarOverviewData = {
  body?: never;
  path: {
    id: number;
  };
  query?: never;
  url: '/api/car/my/{id}/overview';
};

export type GetMyCarOverviewResponses = {
  /**
   * Car found.
   */
  200: CarOverview;
  /**
   * Id was not found.
   */
  204: void;
};

export type GetMyCarOverviewResponse =
  GetMyCarOverviewResponses[keyof GetMyCarOverviewResponses];

export type ImportCsvData = {
  body: {
    file?: Blob | File;
    order?: CsvOrder;
  };
  path: {
    carId: number;
  };
  query?: {
    skipHeader?: boolean;
  };
  url: '/api/import/{carId}';
};

export type ImportCsvErrors = {
  /**
   * - Order is invalid
   * - Car not found
   */
  400: unknown;
  /**
   * - CSV was not set
   * - Something went wrong while importing. Please ask the server admin for help.
   */
  500: unknown;
};

export type ImportCsvResponses = {
  /**
   * CSV was imported.
   */
  202: unknown;
};

export type GetStatsAverageData = {
  body?: never;
  path: {
    carId: number;
  };
  query?: {
    from?: LocalDate;
    to?: LocalDate;
  };
  url: '/api/stats/{carId}/average';
};

export type GetStatsAverageResponses = {
  /**
   * OK
   */
  200: AverageStats;
};

export type GetStatsAverageResponse =
  GetStatsAverageResponses[keyof GetStatsAverageResponses];

export type GetStatsHiLoData = {
  body?: never;
  path: {
    carId: number;
  };
  query?: {
    from?: LocalDate;
    to?: LocalDate;
  };
  url: '/api/stats/{carId}/hi_lo';
};

export type GetStatsHiLoResponses = {
  /**
   * OK
   */
  200: HiLoStats;
};

export type GetStatsHiLoResponse =
  GetStatsHiLoResponses[keyof GetStatsHiLoResponses];

export type GetStatsMinimalData = {
  body?: never;
  path: {
    carId: number;
  };
  query?: {
    from?: LocalDate;
    to?: LocalDate;
  };
  url: '/api/stats/{carId}/minimal';
};

export type GetStatsMinimalResponses = {
  /**
   * OK
   */
  200: MinimalStats;
};

export type GetStatsMinimalResponse =
  GetStatsMinimalResponses[keyof GetStatsMinimalResponses];

export type GetStatsTotalData = {
  body?: never;
  path: {
    carId: number;
  };
  query?: {
    from?: LocalDate;
    to?: LocalDate;
  };
  url: '/api/stats/{carId}/total';
};

export type GetStatsTotalResponses = {
  /**
   * OK
   */
  200: TotalStats;
};

export type GetStatsTotalResponse =
  GetStatsTotalResponses[keyof GetStatsTotalResponses];

export type GetMyselfData = {
  body?: never;
  path?: never;
  query?: never;
  url: '/api/user/me';
};

export type GetMyselfResponses = {
  /**
   * User found.
   */
  200: Account;
};

export type GetMyselfResponse = GetMyselfResponses[keyof GetMyselfResponses];
